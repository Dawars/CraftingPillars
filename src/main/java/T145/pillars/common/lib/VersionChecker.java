package T145.pillars.common.lib;
...
public class VersionChecker {
	public static class Version {
		...
		public boolean greater(Version v) {
			for (int i = 0; i < digits.length && i < v.digits.length; i++)
				if (digits[i] > v.digits[i])
					return true;
				else if (digits[i] < v.digits[i])
					return false;
			return digits.length > v.digits.length;
		}
		...
		public boolean equals(Object o) {
			...
			Version v = (Version) o;
			if (v.digits.length != digits.length)
				return false;
	...
	public static void check() {
		try {
			...
			if (update) {
				...
				int n = JOptionPane.showOptionDialog(new Frame(), "New update available for the Crafting Pillars mod! Do you want to check it out?", "Update is available!", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, buttons, buttons[0]);
