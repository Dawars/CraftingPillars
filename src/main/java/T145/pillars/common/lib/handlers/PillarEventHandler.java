package T145.pillars.common.lib.handlers;
...
import T145.pillars.common.blocks.BaseBlockContainer;
...
public class PillarEventHandler {
	...
	public void onPlayerGetHurt(LivingHurtEvent event) {
		...
		if (event.entityLiving instanceof EntityPlayer
			...
			if (BaublesApi.getBaubles((EntityPlayer) event.entityLiving) != null) {
				...
				boolean isRing1 = false, isRing2 = false;
	...
	protected boolean teleportTo(Entity entity, double targetX, double targetY, double targetZ) {
		double d3 = entity.posX, d4 = entity.posY, d5 = entity.posZ;
		entity.posX = targetX;
		entity.posY = targetY;
		entity.posZ = targetZ;
		boolean flag = false;
		int i = MathHelper.floor_double(entity.posX), j = MathHelper.floor_double(entity.posY), k = MathHelper.floor_double(entity.posZ);
	...
	private void setPositionEntity(Entity entity, double posX, double posY, double posZ) {
		...
		if (player.playerNetServerHandler.func_147362_b().isChannelOpen()) {
			EnderTeleportEvent event2 = new EnderTeleportEvent(player, posX, posY, posZ, 5.0F);
