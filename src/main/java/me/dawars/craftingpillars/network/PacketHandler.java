...
import me.dawars.craftingpillars.init.Config;
...
public class PacketHandler extends MessageToMessageCodec<FMLProxyPacket, PacketBase> {
	...
	public static void preInit() {
		...
		instance.channels = NetworkRegistry.INSTANCE.newChannel("CraftingPillars", instance);
	}
	...
	protected void decode(ChannelHandlerContext ctx, FMLProxyPacket msg, List<Object> out) throws Exception {
		...
		switch (ctx.channel().attr(NetworkRegistry.CHANNEL_SOURCE).get()) {
			case CLIENT:
				player = CraftingPillars.proxy.getClientPlayer();
	...
	private void handlePacketClient(final PacketBase packet, final EntityPlayer player) {
		...
		IThreadListener threadListener = CraftingPillars.proxy.getClientListener();
	...
	private void handlePacketServer(final PacketBase packet, final EntityPlayer player) {
		...
		IThreadListener threadListener = CraftingPillars.proxy.getServerListener();
		if (!threadListener.isCallingFromMinecraftThread()) {
	...
	private void injectPacket(byte[] data) throws Exception {
		...
		handlePacketClient(pkt, CraftingPillars.proxy.getClientPlayer());
	}
	...
	public static void sendToAllAround(PacketBase message, TileEntity theTile) {
		...
		instance.channels.get(Side.SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGET).set(FMLOutboundHandler.OutboundTarget.ALLAROUNDPOINT);
		instance.channels.get(Side.SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGETARGS).set(new TargetPoint(theTile.getWorld().provider.getDimension(), theTile.getPos().getX(), theTile.getPos().getY(), theTile.getPos().getZ(), Config.NETWORK_UPDATE_RANGE));
		instance.channels.get(Side.SERVER).writeAndFlush(message);
	}
	...
	public static void sendToAllAround(PacketBase message, World world, int x, int y, int z) {
		...
		instance.channels.get(Side.SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGET).set(FMLOutboundHandler.OutboundTarget.ALLAROUNDPOINT);
		instance.channels.get(Side.SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGETARGS).set(new TargetPoint(world.provider.getDimension(), x, y, z, Config.NETWORK_UPDATE_RANGE));
		instance.channels.get(Side.SERVER).writeAndFlush(message);
	}
